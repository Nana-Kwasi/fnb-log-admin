...
//   // Updated API URLs
//   // const API_URL = "http://localhost:5001/visitors";
//   const BRANCH_DATA_URL = "http://localhost:5001/visitorslog/visitors/index/branch";
...
export const VisitorProvider = ({ children }) => {
  ...
  const saveBranchDataToCache = (branchCode, branchName, fullData) => {
    const cacheObject = {
      ...fullData,
      selectedBranch: branchCode,
    ...
    localStorage.setItem("branchDataCache", JSON.stringify(cacheObject));
  };

  const loadBranchDataFromCache = () => {
    const cache = localStorage.getItem("branchDataCache");
  ...
  const fetchBranchData = async (branchCode, branchName) => {
    setLoading(true);
    setError("");
    try {
      const headers = {};
      ...
      const groupedData = data.reduce(
        (acc, log) => {
          ...
          if (date.getFullYear() === currentYear) {
            const month = date.toLocaleString("default", { month: "long" });
            acc.monthly[month] = (acc.monthly[month] || 0) + 1;
            if (formatDateForAPI(date) === todayFormatted) {
              acc.today += 1;
            }
            acc.total += 1;
          }
      ...
      const todayVisitors = data.filter((v) => {
        const date = parseAPIDate(v.date);
        return formatDateForAPI(date) === todayFormatted;
      });
      ...
      setBranchData(fullBranchData);
      saveBranchDataToCache(branchCode, branchName, fullBranchData);
  ...
  const login = async (email, branchCode, authToken = null, branchName = "", role = "") => {
    ...
    try {
      if (authToken) {
        ...
        setSelectedBranchName(branchName);
        setAuthenticated(true);
        await fetchBranchData(branchCode, branchName);
        setLoading(false);
        return true;
      }
      ...
      const response = await fetch(`${AUTH_URL}/login`, {
        ...
        body: JSON.stringify({
          email,
          password: "default-needed-in-body",
          branch: branchCode,
        }),
      });
  ...
  const logout = () => {
    localStorage.clear(); // already there, but make sure it clears lastPage
    setToken(null);
    setSelectedBranch("");
    setSelectedBranchName("");
    setBranchData({
  ...
  useEffect(() => {
    const checkAuth = async () => {
      const storedToken = localStorage.getItem("token");
      const storedUser = localStorage.getItem("user");
      const cache = loadBranchDataFromCache();
      ...
      if (storedToken && storedUser) {
        try {
          ...
          if (res.ok) {
            ...
            if (cache) {
              const lastUpdated = new Date(cache.lastUpdated || 0);
              const today = new Date();
              if (lastUpdated.toDateString() === today.toDateString()) {
                setBranchData({
                  analyticsData: cache.analyticsData || [],
                  totalVisitors: cache.totalVisitors || 0,
                  visitorsToday: cache.visitorsToday || 0,
                  todayVisitorsData: cache.todayVisitorsData || [],
                  allVisitorsData: cache.allVisitorsData || [],
                });
              } else {
                fetchBranchData(userData.branchCode, userData.branchName);
              }
            } else {
              fetchBranchData(userData.branchCode, userData.branchName);
            }
