...
export const VisitorProvider = ({ children }) => {
  ...
  // Parse user profile data from verify2fa API response
  ...
  // Modified login function to include user profile information
  const login = async (email, branchCode, authToken = null, branchName = "", role = "", verifyResponseData = null) => {
    ...
    try {
      ...
      if (authToken) {
        ...
        // Parse and set user profile if verify2fa response data is provided
        if (verifyResponseData) {
          const profileData = parseUserProfileFromResponse(verifyResponseData);
          if (profileData) {
            setUserProfile(profileData);
            localStorage.setItem('userProfile', JSON.stringify(profileData));
            console.log("User profile data saved:", profileData);
          }
        }
          ...
          if (response.ok) {
            ...
            if (storedUserProfile) {
              try {
                ...
                setUserProfile(profileData);
                console.log("Restored user profile from localStorage:", profileData);
              } catch (err) {
