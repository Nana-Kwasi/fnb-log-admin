...
export const VisitorProvider = ({ children }) => {
  ...
  const fetchBranchData = async (branchName) => {
    ...
    try {
      ...
      // Use the new branch-specific endpoint
      const response = await fetch(`${API_URL}/index?branch=${encodeURIComponent(branchName)}`, {
        method: 'GET',
        headers: {
          'x-auth-token': token,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to fetch data for branch: ${branchName}`);
      }
      
      // Parse response data
      const branchData = await response.json();
      console.log(`Fetched ${branchData.length} entries for branch: ${branchName}`);
      
      // If no data found, set appropriate state
      if (branchData.length === 0) {
        console.log(`No visitor logs found for branch: ${branchName}`);
        setBranchData({
          analyticsData: [],
          totalVisitors: 0,
          visitorsToday: 0,
      ...
      const processedData = branchData.reduce((acc, log) => {
        ...
        try {
          ...
          if (logDate.getFullYear() === currentYear) {
            ...
            acc.monthlyData[month] = (acc.monthlyData[month] || 0) + 1;
            
            // Today's visitors
            const formattedLogDate = formatDateForAPI(logDate);
            if (formattedLogDate === todayFormatted) {
              acc.todayVisitors.push(log);
              acc.todayCount++;
            }
          ...
          console.error('Error processing log entry:', parseError);
          return acc;
        }
      }, {
        monthlyData: {},
      ...
      const finalBranchData = {
        ...
        totalVisitors: processedData.totalCount,
        visitorsToday: processedData.todayCount,
        todayVisitorsData: processedData.todayVisitors,
        allVisitorsData: branchData
      };
      
      // Update context state
      setBranchData(finalBranchData);
      
      // Store in localStorage for persistence
      const dashboardPersistenceData = {
        ...finalBranchData,
        lastUpdated: new Date().toISOString(),
        selectedBranch: branchName
      };
      
      localStorage.setItem("dashboardData", JSON.stringify(dashboardPersistenceData));
      
      setLoading(false);
      return true;
    } catch (err) {
      console.error("Comprehensive error in fetchBranchData:", err);
      
      // Reset branch data on error
      setBranchData({
        analyticsData: [],
        totalVisitors: 0,
        visitorsToday: 0,
      ...
      // Set error message
      setError(err.message || "Failed to fetch branch data. Please try again.");
      setLoading(false);
      return false;
    }
  };
