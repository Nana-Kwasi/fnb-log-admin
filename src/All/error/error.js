...
const authenticateUser = async (req, res) => {
  ...
  try {
    ...
    // Make the authentication request to LDAP service
    ...
    if (!authResponse.data || 
        (authResponse.data.status_code !== "0" && authResponse.data.status_code !== 0)) {
...
const verify2FA = async (req, res) => {
  ...
  try {
    ...
    // Make the verification request to LDAP service
    ...
    if (!verifyResponse.data || 
        (verifyResponse.data.status_code !== "0" && verifyResponse.data.status_code !== 0)) {
...
router.post('/verify', userController.verifyFnumber);

// New authentication routes
router.post('/authenticate', userController.authenticateUser);
router.post('/verify2fa', userController.verify2FA);
router.post('/finalize-login', userController.finalizeLogin);
