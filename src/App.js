...
// const AppContent = ({ handleLogin, userEmail }) => {
//   const { user, authenticated, hydrating } = useVisitor();
//   const [isAdmin, setIsAdmin] = useState(false);
...
//       <Router>
//         <div className="app">
//           <AppContent handleLogin={handleLogin} userEmail={userEmail} />
//         </div>
//       </Router>
//     </VisitorProvider>
//   );
...
const AppContent = ({ handleLogin, userEmail }) => {
  ...
  // Allow detail page route pattern in lastPage check
  const validRoutes = [
    "/",
    "/visitor-logs",
    "/logbook",
  ...
  const isValidRoute = (path) => {
    ...
    // Allow dynamic detail page
    return /^\/visitor-details\/[A-Za-z0-9_-]+$/.test(path);
  };
  ...
  // Save last visited page only if valid
  useEffect(() => {
    if (authenticated) {
  ...
  // Redirect after login with valid route handling
 // Inside AppContent, change this effect:
useEffect(() => {
  if (authenticated && !hydrating) {
    let lastPage = localStorage.getItem("lastPage");

    if (!lastPage || !isValidRoute(lastPage)) {
      localStorage.removeItem("lastPage");
      lastPage = "/";
    }
